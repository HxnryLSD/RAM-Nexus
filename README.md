...
- **Code signing**: binaries ship **unsigned** by default — expect the normal
  SmartScreen "More info → Run anyway" prompt on first run until the download builds
  reputation, and a possible generic antivirus heuristic flag because the app is
  elevated and writes into Roblox's install folder. To sign with a real CA cert,
  set `CERT_PFX`/`CERT_PASSWORD` when running `scripts/build-installer.sh`.
